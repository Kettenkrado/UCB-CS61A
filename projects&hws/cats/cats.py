...
def accuracy(typed, source):
    ...
    if len_typed == 0 and len_source == 0:
        return 100.0
    elif len_typed == 0 or len_source == 0:
        return 0.0
    match = 0
    compare = min(len_typed, len_source)
    for i in range(compare):
        if typed_words[i] == source_words[i]:
            match += 1
    return match / len_typed * 100.0
    # END PROBLEM 3
...
def feline_fixes(typed, source, limit):
    ...
    if length_diff > limit:
        return limit + 1
    def diff(typed, source, difference):
        if len(typed) == 0 or len(source) == 0 or length_diff + difference > limit:
            return length_diff + difference
        else:
            if typed[0] != source[0]:
                difference += 1
            return diff(typed[1:], source[1:], difference)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    word_num = len(words)
    player_num = len(timestamps_per_player)
    times = []
...
def fastest_words(match):
    ...
    for i in word_indices:
        min_time = max(max(times)) + 1
        for j in player_indices:
            if times[j][i] < min_time:
                player = j
                min_time = times[j][i]
        fastest[player].append(words[i])
    return fastest
    # END PROBLEM 10
