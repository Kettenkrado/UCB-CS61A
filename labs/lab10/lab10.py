...
def floor_div(args):
    ...
    operands = args.rest
    while operands != nil:
        result = result // calc_eval(operands.first)
...
def eval_and(expressions):
    ...
    True
    """
    if expressions is nil: # special case
        return True
    if expressions.rest is nil: # base case
        if calc_eval(expressions.first) is not scheme_f:
